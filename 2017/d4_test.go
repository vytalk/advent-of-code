...
var d4DataSetA = []Passphrases{
	{[]string{"aa bb cc dd ee", "aa bb cc dd aa", "aa bb cc dd aaa"}, 2},
	{[]string{"sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp",
...
func TestPassphraseValidatorA(t *testing.T) {
	for _, data := range d4DataSetA {
		if result := ValidatePassphrases(data.passphareses); data.expected != result {
			t.Errorf("Expected score of %d, but it was %d instead.", data.expected, result)
		}
	}
}

var d4DataSetB = []Passphrases{
	{[]string{"abcde fghij", "abcde xyz ecdab", "a ab abc abd abf abj",
		"iiii oiii ooii oooi oooo", "oiii ioii iioi iiio"}, 3},
	{[]string{"sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp",
		"xmuf znkhaes pggrlp zia znkhaes znkhaes",
		"nti rxr bogebb zdwrin",
		"sryookh unrudn zrkz jxhrdo gctlyz",
		...
		"ydjhhf yeltadb lwi cjdcb ovaox xrdm vkxub",
		"zax xza admbc lvpzfeh auxn rwasj",
		"kebx eild nrskdr meja jxczomh gcne"}, 265}}

func TestPassphraseValidatorB(t *testing.T) {
	for _, data := range d4DataSetB {
		if result := ValidatePassphrasesB(data.passphareses); data.expected != result {
			t.Errorf("Expected score of %d, but it was %d instead.", data.expected, result)
		}
	}
}
